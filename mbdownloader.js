...
(function($, undefined) {
    ...
    $.widget('nntoan.mbDownloader', {
        ...
        options: {
            ...
            insertMode: 'appendTo',
            credits: '<p>Truyá»‡n Ä‘Æ°á»£c táº£i tá»« MeTruyenChu</p><p>Nháº­n báº£n má»›i nháº¥t táº¡i: <a href="https://vetabook.blogspot.com">VetaBook.blogspot.com</a></p>',
            general: {
                host: location.host,
            ...
            processing: {
                ebookFileName: null,
                ebookFileExt: '.epub',
                documentTitle: '[...] Vui lÃ²ng chá» trong giÃ¢y lÃ¡t',
            },
            regularExp: {
                chapter: /\s*ChÆ°Æ¡ng\s*\d+\s?:.*[^<\n]/g,
                novel: /\s*Tiá»ƒu\s*thuyáº¿t\s?:.*[^<\n]/g,
                ...
                eoctext: ['(ps:|hoan nghÃªnh quáº£ng Ä‘áº¡i báº¡n Ä‘á»c quang lÃ¢m|Huyá»n áº£o khoÃ¡i trÃ­ Ã¢n cá»«u)', 'i'],
                breakline: /\n/g,
                chapList: /(?:href=")[^")]+(?=")/g,
            },
            classNames: {
            ...
            ebook: {
                title: null,
                author: null,
                publisher: 'VetaBook.blogspot.com',
                description: null,
                fallbackCover: null,
            ...
            $ajax.done(function (response) {
                if (options.isGlocCallbackRequired) {
                    $.ajax(options.xhr.chapter).done(function(data) {
                        ...
                        $widget.text('Lá»—i trÆ°á»›c khi bá»‹ lá»—i danh má»¥c :)');
        ...
        processListOfChapters: function (response, that, $widget) {
            ...
            if (event.type === 'contextmenu') {
                $widget.off('click');
                var startFrom = prompt('Nháº­p ID chÆ°Æ¡ng truyá»‡n báº¯t Ä‘áº§u táº£i:', options.chapters.chapList[0]);
            ...
            if (that.processing.chapListSize > 0) {
                that.elements.$window.on('beforeunload', function () {
                    return 'Truyá»‡n Ä‘ang Ä‘Æ°á»£c táº£i xuá»‘ng...';
                });
        ...
        getContent: function ($widget) {
            ...
            $.ajax(options.xhr.content).done(function (response) {
                ...
                $widget.html('Äang táº£i: ' + Math.floor((self.processing.count / self.processing.chapListSize) * 100) + '%');
                ...
                self.downloadError('Káº¿t ná»‘i khÃ´ng á»•n Ä‘á»‹nh', error);
        ...
        updateChapterTitle: function (that, $result) {
            ...
            if (options.chapters.chapTitle === '') {
                options.chapters.chapTitle = 'ChÆ°Æ¡ng ' + chapNum;
            }
        ...
        parseChapterContent: function (that, $chapter, $widget) {
            ...
            if (!$chapter.length) {
                chapContent = that.downloadError('KhÃ´ng cÃ³ ná»™i dung');
            } else {
                if ($chapter.find(options.classNames.chapterVip).length) {
                    chapContent = that.downloadError('ChÆ°Æ¡ng VIP');
                } else if ($chapter.filter(function () {
                    return (this.textContent.toLowerCase().indexOf('vui lÃ²ng Ä‘Äƒng nháº­p Ä‘á»ƒ Ä‘á»c chÆ°Æ¡ng nÃ y') !== -1);
                }).length) {
                    chapContent = that.downloadError('ChÆ°Æ¡ng yÃªu cáº§u Ä‘Äƒng nháº­p');
                } else {
                    var $img = $chapter.find('img');
                    if ($img.length) {
                        $img.replaceWith(function () {
                            return '<br /><a href="' + this.src + '">Click Ä‘á»ƒ xem áº£nh</a><br />';
                        });
                    }
                    ...
                    if ($chapter.text().trim() === '') {
                        chapContent = that.downloadError('Ná»™i dung khÃ´ng cÃ³');
                    } else {
        ...
        downloadError: function (error, message, $widget, retry) {
            ...
            if (options.errorAlert) options.errorAlert = confirm('Lá»—i! ' + message + '\nBáº¡n cÃ³ muá»‘n tiáº¿p tá»¥c nháº­n cáº£nh bÃ¡o?');
        ...
        saveEbook: function ($widget) {
            ...
            $widget.html('Äang nÃ©n EPUB...');

            if (self.processing.titleError.length) {
                self.processing.titleError = '<p class="no-indent"><strong>CÃ¡c chÆ°Æ¡ng lá»—i: </strong>' + self.processing.titleError.join(', ') + '</p>';
            } else {
                self.processing.titleError = '';
            }
            self.processing.beginEnd = '<p class="no-indent">Ná»™i dung tá»« <strong>' + self.processing.begin + '</strong> Ä‘áº¿n <strong>' + self.processing.end + '</strong></p>';
